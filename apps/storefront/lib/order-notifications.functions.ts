...
export const notifyNewOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { hasSupabaseAdminCredentials, supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    // Send the customer invoice by email (best-effort; independent of the
    // automation cron, which handles marketing/status notifications).
