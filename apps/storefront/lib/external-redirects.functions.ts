 ...
 * so the admin-configured cross-app URLs are already in cross-app-urls.ts's
 * cache *before* any page component renders — including the very first SSR
 * pass, which is what actually matters here. A client-only `useEffect` (the
 * old approach) is always too late for that first paint, since the HTML the
 * browser receives is generated in Node.
 */
