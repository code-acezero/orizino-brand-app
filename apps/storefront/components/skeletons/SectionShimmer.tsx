        ...
        <div className={`grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5 sm:gap-4 ${className}`}>
          {Array.from({ length: count }).map((_, i) => (
            <div key={i} className="rounded-2xl border border-border/30 bg-card/30 overflow-hidden">
              <div className="relative w-full overflow-hidden flex items-center justify-center" style={{ aspectRatio: "5/6" }}>
        ...
        <div className={`flex gap-3 overflow-hidden ${className}`}>
          {Array.from({ length: count }).map((_, i) => (
            <div key={i} className="shrink-0 w-[160px] sm:w-[200px] rounded-2xl border border-border/30 bg-card/30 overflow-hidden">
              <div className="relative w-full overflow-hidden flex items-center justify-center" style={{ aspectRatio: "5/6" }}>
                <SkBlock className="w-full h-full rounded-none absolute inset-0" />
                <SkeletonWatermark size="sm" className="z-10" />
              </div>
    ...
    case "productHero":
      return (
        <div className={`grid md:grid-cols-2 gap-4 md:gap-8 ${className}`}>
          <div className="relative rounded-2xl md:rounded-3xl w-full overflow-hidden flex items-center justify-center bg-card/30 border border-border/30" style={{ aspectRatio: "1/1" }}>
            <SkBlock className="w-full h-full rounded-none absolute inset-0" />
