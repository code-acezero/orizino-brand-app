...
export default function DashboardSalesChart({ isCompact = false }: { isCompact?: boolean }) {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="border border-border/80 bg-background/95 p-2.5 shadow-xl text-xs space-y-1 min-w-[140px]">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="flex items-center border border-border/60">
            ...
            <button
              ...
              className={`px-2.5 py-1 text-[11px] font-semibold transition-colors cursor-pointer border-l border-border/60 ${
                metric === "orders"
                  ? "bg-foreground text-background"
                  : "text-muted-foreground hover:text-foreground bg-transparent"
              }`}
          ...
          <div className="flex items-center border border-border/60">
            {[7, 14, 30].map((d, idx) => (
              <button
                key={d}
                onClick={() => setDays(d)}
                className={`px-2 py-1 text-[10px] font-mono font-semibold transition-colors cursor-pointer ${
