...
export interface QuadSyncResult {
  ok: boolean;
  totalUnified: number;
  ...
  sheetsPushed: boolean;
  sheetsCount?: number;
  newSubscribersAdded: number;
  newAudienceAdded: number;
  timestamp: string;
  error?: string;
}
...
/**
 * Executes a full 4-way synchronization across:
 * 1. Subscribers (`email_subscriptions`)
 * 2. Database Profiles / Auth Users (`auth.users` & `profiles`)
 * 3. Marketing Audience Table (`marketing_audience_table` in `site_settings`)
 * 4. Google Sheets (`marketing_audience_sheet_url`)
 */
export async function executeQuadSync(rawOptions?: {
  pushToSheetsIfConfigured?: boolean;
  pullFromSheetsIfConfigured?: boolean;
}): Promise<QuadSyncResult> {
  const options = (rawOptions as any)?.data || rawOptions || {
    pushToSheetsIfConfigured: true,
    pullFromSheetsIfConfigured: true,
  };
  ...
  try {
    ...
    const existingAudience: UnifiedContact[] = Array.isArray(audienceSettings?.value)
      ? (audienceSettings.value as any)
      : [];
    ...
    // Unification Key -> UnifiedContact
    ...
    const getOrInit = (key: string, initial: Partial<UnifiedContact>): UnifiedContact => {
    ...
    let sheetsCount = 0;
    if (sheetUrl && options?.pullFromSheetsIfConfigured) {
      try {
    ...
    // 8. Push to Google Sheets if connected and enabled
    let sheetsPushed = false;
    if (sheetUrl && options?.pushToSheetsIfConfigured) {
      try {
        const spreadsheetId = extractSheetId(sheetUrl);
        if (spreadsheetId) {
          const token = await googleAccessToken(GOOGLE_SCOPES.sheets);
    ...
    return {
      ...
      totalUnified: 0,
      subscribersCount: 0,
      databaseUsersCount: 0,
      ordersCount: 0,
      audienceCount: 0,
      sheetsPushed: false,
      newSubscribersAdded: 0,
      newAudienceAdded: 0,
      timestamp: new Date().toISOString(),
