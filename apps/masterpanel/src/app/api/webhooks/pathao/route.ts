...
// ---------------------------------------------------------------------------
// Official Pathao Courier Merchant API v1 Webhook Handler (Masterpanel)
...
export async function POST(req: NextRequest) {
  ...
  // ── Official Webhook Integration Handshake Probe ─────────────────────────
  if (eventName === "webhook_integration" || (!eventName && payload?.consignment_id === undefined && payload?.merchant_order_id === undefined)) {
    console.log("[masterpanel-pathao-webhook] Handshake Probe Received. Returning 202 Accepted.");
    return new NextResponse(
      JSON.stringify({
        status: "success",
