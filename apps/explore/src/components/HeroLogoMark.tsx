...
export function HeroLogoMark({
  ...
  onClick?: () => void;
  wholeAnimate?: TargetAndTransition;
  wholeTransition?: Transition;
  shardAnimations?: ShardAnim[];
}) {
  return (
    <motion.svg
      ...
      onClick={onClick}
      animate={wholeAnimate}
      transition={wholeTransition}
      xmlns="http://www.w3.org/2000/svg"
    >
      {HERO_PATHS.map((d, i) => {
        const a = shardAnimations?.[i];
        return (
          <motion.path
            key={i}
            d={d}
            ...
            animate={a ? { x: a.x, y: a.y, rotate: a.rotate, scale: a.scale, opacity: a.opacity } : undefined}
