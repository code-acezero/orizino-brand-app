...
export function sampleInvoicePayload(brandOverrides?: Partial<InvoiceOrderPayload["brand"]>): InvoiceOrderPayload {
  return {
    ...
    brand: {
      ...
      address: "Flagship Atelier & Head Office, Dhaka",
      email: "concierge@orizino.com",
      phone: "+880 1800-000000",
      website: "www.orizino.com",
      ...brandOverrides,
    },
    customer: {
      ...
      billing_address: "House 12, Road 5, Sector 3, Uttara, Dhaka 1230",
      shipping_address: "House 12, Road 5, Sector 3, Uttara, Dhaka 1230",
    },
    items: [
      {
...
function wrapDoc(s: InvoiceSettings, body: string, order?: InvoiceOrderPayload): string {
  ...
  let baseQrPx = 168;
  if (s.qr_size === "compact") baseQrPx = 130;
  else if (s.qr_size === "medium") baseQrPx = 155;
  else if (s.qr_size === "large") baseQrPx = 180;
  else if (s.qr_size === "full_width") baseQrPx = 205;
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Symmetrical 3-Column Parallel Info Grid with Vertical Dividers ── */
  .imp-info-grid {
    display: grid;
  ...
  /* Col 1: Left-aligned Brand Block */
  .imp-brand-block {
    display: flex;
    flex-direction: column;
  ...
  /* Col 2: Center-aligned Customer / Bill To Block */
  .imp-customer-block {
    display: flex;
    flex-direction: column;
    align-items: center;
  ...
  /* Col 3: Right-aligned Invoice Meta Block */
  .imp-invoice-meta-block {
    display: flex;
    flex-direction: column;
  ...
  .imp-meta-table td.label {
    ...
    color: var(--text);
    font-weight: 700;
    font-size: ${(Number(finalCustRem) * 0.88).toFixed(2)}rem;
...
function renderImperialBody(s: InvoiceSettings, o: InvoiceOrderPayload): string {
  ...
  // ── 2. Seller, Customer & Invoice Details 3-Column Parallel Grid ──
  parts.push(`<div class="imp-info-grid">`);
  
  // Left: Official Brand Vector Crest + Name + Address + Contact (Left-Aligned)
  ...
  parts.push(`<div class="imp-brand-text-col">`);
  parts.push(`<div class="imp-brand-name">${escapeHtml(o.brand.name || "ORIZINO")}</div>`);
  ...
  parts.push(`<div class="imp-brand-contact-line">${escapeHtml(o.brand.phone || "+880 1800-000000")}</div>`);
  if (o.brand.email) {
  ...
  // Center: Bill To / Customer Info (Center-Aligned)
  parts.push(`<div class="imp-customer-block">`);
  parts.push(`<div class="imp-section-label">BILL TO:</div>`);
  parts.push(`<div class="imp-customer-name">${escapeHtml(o.customer.full_name || "Customer Name")}</div>`);
  ...
  if (o.customer.phone) {
    parts.push(`<div class="imp-brand-line">${escapeHtml(o.customer.phone)}</div>`);
  }
  if (o.customer.email) {
    parts.push(`<div class="imp-brand-line">${escapeHtml(o.customer.email)}</div>`);
  }
  parts.push(`</div>`);

  // Right: Invoice Details (Right-Aligned)
  parts.push(`<div class="imp-invoice-meta-block">`);
  parts.push(`<div class="imp-section-label">INVOICE DETAILS</div>`);
  parts.push(`<table class="imp-meta-table">`);
  parts.push(`<tr><td class="label">INVOICE NO:</td><td class="val font-mono">${escapeHtml(o.invoice_number || o.order_number)}</td></tr>`);
  ...
  parts.push(`</div>`);
  parts.push(`<div class="imp-qr-footer-banner"><div>Scan to verify</div><div>genuine order details</div></div>`);
  parts.push(`</div>`);
  ...
  // Online Concierge, Links & Official Postal Stamp
  const rawWeb = (o.brand.website || "orizino.com").replace(/^https?:\/\//, "").replace(/\/$/, "");
  const brandSiteUrl = rawWeb || "orizino.com";

  parts.push(`<div class="imp-payinfo-and-stamp">`);
  parts.push(`<div>`);
  parts.push(`<div class="imp-section-label">ONLINE &amp; SUPPORT</div>`);
  parts.push(`<div class="imp-pay-text">`);
  parts.push(`<div>Web: <span style="font-weight:700; color:var(--text);">${escapeHtml(brandSiteUrl)}</span></div>`);
  if (o.brand.email) {
    parts.push(`<div>Support: <span style="font-weight:700; color:var(--text);">${escapeHtml(o.brand.email)}</span></div>`);
  }
  if (o.brand.phone) {
    parts.push(`<div>Mobile: <span style="font-weight:700; color:var(--text);">${escapeHtml(o.brand.phone)}</span></div>`);
  }
  parts.push(`</div>`);
  parts.push(`</div>`);
