...
export async function POST(req: NextRequest) {
  try {
    ...
    if (checkError) {
      console.error("[subscribe-brandhome] checkError:", checkError);
      ...
      .upsert(
        {
          ...
          name: subscriberName,
          is_active: true,
          source: body?.source || "brandhome_landing",
    ...
    if (upsertError) {
      console.error("[subscribe-brandhome] upsertError:", upsertError);
    ...
    try {
      ...
      console.warn("[subscribe-brandhome] audience sync error:", audErr);
    }
    ...
    try {
      ...
      console.error("[subscribe-brandhome] Resend email send error:", sendErr);
    }
