import { createFileRoute, useSearch } from "@orizino/shared/lib/router-compat";
import { useEffect, useState } from "react";
import { useServerFn } from "@/lib/server-fn-compat";
import { trackOrder, trackOrderByToken } from "@/lib/track.functions";
import { z } from "zod";

const TrackSearch = z.object({
  t: z.string().optional(), // tracking token from signed link
});

export const Route = createFileRoute("/track")({
  validateSearch: (s) => TrackSearch.parse(s),
  component: TrackPage,
  head: () => ({
    meta: [
      { title: "Track your order" },
      { name: "description", content: "Look up the status of your order using your order number and email or phone." },
...
function TrackPage() {
  const search = useSearch() as { t?: string };
  const doTrack = useServerFn(trackOrder);
  const doTrackByToken = useServerFn(trackOrderByToken);
  ...
  useEffect(() => {
    if (search.t) {
      setLoading(true);
      doTrackByToken({ data: { token: search.t } })
        .then((res) => {
          if (res.found) setResult(res.order as OrderInfo);
          else setError("This tracking link is invalid or has expired.");
        })
        .catch(() => setError("Something went wrong. Please try again."))
        .finally(() => setLoading(false));
    }
  }, [search.t, doTrackByToken]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setResult(null);
    try {
      const res = await doTrack({ data: { order_number: orderNumber.trim(), contact: contact.trim() } });
      if (res.found) setResult(res.order as OrderInfo);
      else setError("We couldn't find an order matching that. Double-check your order number and the email/phone you used at checkout.");
    } catch (e) {
