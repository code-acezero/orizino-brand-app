...
export default function EmployeeIdentityPage() {
  ...
  const search = useSearch() as { preview?: string };
  ...
  const cardBg = "border-border/40 bg-card/40 backdrop-blur-md";
  const subtleText = "text-muted-foreground";
  const bodyText = "text-foreground/90";
  const chipBg = "bg-card/50 border-border/40 backdrop-blur-sm";
  const hoverChip = "hover:bg-accent hover:text-accent-foreground hover:border-accent";
  ...
  return (
    ...
    >
            ...
            <div className="id-avatar-wrap shrink-0 -mt-16 sm:-mt-20">
              {identity.avatar_url ? (
                <img
                  ...
                  className={`w-28 h-28 md:w-32 md:h-32 rounded-2xl object-cover ring-4 shadow-xl ring-background`}
                />
              ) : (
                <div
                  className={`w-28 h-28 md:w-32 md:h-32 rounded-2xl flex items-center justify-center text-3xl font-bold ring-4 shadow-xl ring-background text-white`}
      ...
      {qrOpen && (
        ...
        >
          <div
            className={`rounded-3xl p-6 max-w-sm w-full text-center border bg-card border-border`}
